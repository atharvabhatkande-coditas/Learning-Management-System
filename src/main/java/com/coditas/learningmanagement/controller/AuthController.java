...
public class AuthController {
    ...
    @PostMapping("/refresh")
    public ResponseEntity<ApplicationResponse<SingleResponse>> generateAccessToken(@Valid @RequestBody GeneralRequest generalRequest){
        ApplicationResponse<SingleResponse> applicationResponse=new ApplicationResponse<>(authService.generateAccessToken(generalRequest));
        return new ResponseEntity<>(applicationResponse, HttpStatus.OK);
    }

    @PostMapping("/logout")
    public ResponseEntity<ApplicationResponse<SingleResponse>> generateAccessToken(){
        ApplicationResponse<SingleResponse> applicationResponse=new ApplicationResponse<>(authService.logout());
        return new ResponseEntity<>(applicationResponse, HttpStatus.OK);
    }
